...
html = html.replace('<div class="form-actions">\n                        <button type="button" class="btn btn-outline" onclick="closeModal(\'modal-consultation\')">Cancel</button>', '<div class="form-actions">\n                        <button type="button" id="btn-delete-consultation" class="btn btn-outline" style="color: #ef4444; border-color: #fca5a5; display: none;" onclick="deleteConsultation()">Delete</button>\n                        <button type="button" class="btn btn-outline" onclick="closeModal(\'modal-consultation\')">Cancel</button>')

html = html.replace('                    <h1 style="font-size: 32px; font-weight: 800; margin: 0; color: #1e293b; letter-spacing: 2px;">IEG</h1>\n                    <span style="font-size: 10px; color: #64748b; text-transform: uppercase;">Industrial<br>Exchange<br>Group</span>', '                    <img src="../../assets/images/IEG_logo.png" style="height: 65px; object-fit: contain; filter: brightness(0);" alt="IEG Logo">')

html = re.sub(r'(?s)    <!-- Confirm Delete Modal -->\n\s*<div id="modal-confirm-delete".*?</div>\n\s*</div>\n\s*</div>\n\s*', '', html)

modals = '''    <!-- Confirm Delete Consultation Modal -->
