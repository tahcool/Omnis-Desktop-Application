...
(function () {
    ...
    window.addQuotationItemRow = function () {
        ...
        // --- Product search using window.supabase preload proxy (always available) ---
        ...
        const doSearch = async (val) => {
            ...
            if (val === _lastVal && suggestBox.innerHTML !== "") return;
            _lastVal = val;

            suggestBox.innerHTML = '<div style="padding:8px; color:#64748b; font-size:11px; text-align:center;"><i class="fa fa-spinner fa-spin"></i> Searching...</div>';
            ...
            try {
                ...
                // Use window.supabase preload proxy (same as quick quotes)
                if (window.supabase && window.supabase.from) {
                    const { data, error } = await window.supabase.from('stock_inventory').select('*').or(orFilter).limit(15);
                ...
                if (results.length > 0) {
                    ...
                    suggestBox.querySelectorAll('.suggest-item').forEach(el => {
                        el.onclick = (e) => {
                            ...
                            if (!raw) return;
                            codeInp.value = raw.model || raw.item_code || raw.name || '';
                            row.querySelector('.item-name').value = raw.item_name || raw.model || raw.name || '';
                            row.querySelector('.item-desc').value = raw.description || '';
                            if (raw.rate) {
                                row.querySelector('.item-rate').value = raw.rate;
                ...
                console.error("[QtnItem] Search error:", e);
                suggestBox.classList.add('hidden');
            }
        };
        ...
        codeInp.addEventListener('input', (e) => {
            clearTimeout(_timer);
            _timer = setTimeout(() => doSearch(e.target.value), 300);
        });
        codeInp.addEventListener('focus', () => { if (codeInp.value.trim()) doSearch(codeInp.value); });
        document.addEventListener('click', (e) => {
            if (e.target !== codeInp && !suggestBox.contains(e.target)) suggestBox.classList.add('hidden');
        }, { capture: true });
    };
