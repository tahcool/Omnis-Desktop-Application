...
def get_data_frame():
	...
	WHERE 
        workflow_state = "{WORKFLOW_QUOTATION_LOST_STAGE}"
    ...
    if not df.empty:
        reason_counts = df["lost_reason"].value_counts()
        reason_percentages = reason_counts / len(df) * 100
        for reason, percentage in reason_percentages.iteritems():
            df[reason] = f"{percentage:.1f}%"

        for reason in ["Other", "VC Fixing Inhouse", "Too Expensive"]:
