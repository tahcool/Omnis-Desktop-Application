...
def get_dataframe(filters=None):
    sql = f"""SELECT 
        ...
        bd.status as "Status",
        -- 🔧 change format to DD/MM/YYYY (no trailing dot in month)
        DATE_FORMAT(bd.quote_sent_date, "%d/%m/%Y") as "Quotation Sent Date",
        bd.resp as "Resp",
        ...
        WHERE bd.end_date IS NULL and bd.resp = 'FSD' AND ma.region = "{filters.get("region")}"
    ...
    if not df.empty:
        date_columns = [
            ...
            "Outwork ETA",
            "Date",
            "Quotation Sent Date",  # keep it here; pandas can parse 06/10/2025 just fine
        ]
