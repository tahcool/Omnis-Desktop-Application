...
def get_data_frame():
	...
	WHERE 
        workflow_state not in {(WORKFLOW_STATE_COMPLETED, WORKFLOW_QUOTATION_LOST_STAGE)}
        and jrv.responsibility = "FSD"
    ...
    if not df.empty:
        ...
        for status, percentage in status_percentages.iteritems():
            df[status] = f"{percentage:.1f}%"

        for state in [s for s in ALL_WORKFLOW_STATES if s not in status_counts.keys()]:
            df[state] = "0.0%"
...
def get_data(df, filters=None):
    ...
    # data = df.loc[:, ~df.columns.isin(ALL_WORKFLOW_STATES)].values.tolist()
    return data
