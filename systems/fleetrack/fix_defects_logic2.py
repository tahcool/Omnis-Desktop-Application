...
"""
c = re.sub(r'async function loadFtDefectsDashboard\(\) \{.*?updateDefectFilterSummary\(\);\s*renderDefectTable\(\);\s*\} catch \(e\) \{.*?\}', new_kpi_logic.strip(), c, count=1, flags=re.DOTALL)
...
"""
c = re.sub(r'async function loadFtDefects\(\) \{.*?renderDefectsTable\(FT_DEFECTS_DATA\);\s*\} catch \(e\) \{.*?\}', new_list_logic.strip(), c, count=1, flags=re.DOTALL)
...
"""
c = re.sub(r'async function saveDefect\(\) \{.*?loadFtDefects\(\);\s*loadFtDefectsDashboard\(\);\s*\} catch \(e\) \{.*?\}', new_save_logic.strip(), c, count=1, flags=re.DOTALL)

with open('index.html', 'w', encoding='utf-8') as f:
    f.write(c)

print("Applied defects logic!")
