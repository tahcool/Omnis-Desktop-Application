...
# Fix the Powertrack error
content = content.replace(
    "const res = await callFrappe('/api/method/ptz_powertrack.omnis_dashboard.pt_dashboard.get_report_schedule', {});",
    "// Powertrack API call removed for Supabase Migration\n            const res = { message: { schedule: [] } };"
)

# Insert loadFtBreakdownDashboard
# Find where we initialize the dashboard or a good place to put it
injection_marker = "async function checkUnsentSupervisorApprovedBreakdowns() {"
        ...
        try {
            ...
            if (tbody) {
                ...
                if (!rowsToShow.length) {
                    tbody.innerHTML = <tr><td colspan="6" style="font-size:11px;color:#6b7280;text-align:center;">No open breakdowns.</td></tr>;
                } else {
                    rowsToShow.forEach((row, idx) => {
                        ...
                        tr.innerHTML = 
                            ...
                            <td></td>
                            <td></td>
                            <td><button class="btn btn-secondary btn-sm" onclick="window.viewBreakdown('')">View</button></td>
                        ;
...
if "async function loadFtBreakdownDashboard()" not in content:
    content = content.replace(injection_marker, dashboard_code + injection_marker)

# Make sure it gets called on load
# Find where checkUnsentSupervisorApprovedBreakdowns is called
call_marker = "checkUnsentSupervisorApprovedBreakdowns();"
if "loadFtBreakdownDashboard();" not in content:
    content = content.replace(call_marker, "loadFtBreakdownDashboard();\n      " + call_marker)
