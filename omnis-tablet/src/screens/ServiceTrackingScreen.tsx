...
export default function ServiceTrackingScreen({ navigation }: any) {
  ...
  const fetchData = async () => {
    setLoading(true);
    let q = supabase.from('ft_service_plan').select('*');
  ...
  const handleAdd = async () => {
    ...
    const { error } = await supabase.from('ft_service_plan').insert([{ ...newItem , status: 'Scheduled' }]);
    if (!error) {
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <View style={styles.tableRow}>
                    ...
                    <View style={styles.cell}>
                      <Text style={styles.cellText} numberOfLines={2}>{item.date || '-'}</Text>
                    </View>
                    <View style={styles.cell}>
        ...
        <View style={styles.modalOverlay}>
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalContainer}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Add Service Tracking</Text>
            ...
            <TextInput style={styles.input} placeholder="e.g. 500H Service" placeholderTextColor="#64748b" onChangeText={(t: string) => setNewItem({...newItem, description: t})} />
            
            <Text style={styles.inputLabel}>Date</Text>
            <TextInput style={styles.input} placeholder="YYYY-MM-DD" placeholderTextColor="#64748b" onChangeText={(t: string) => setNewItem({...newItem, date: t})} />
