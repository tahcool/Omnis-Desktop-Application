...
# 1. Fix Deep Linking
oldDeepLink = """          if (url && url.includes('omnis://quote/')) {
...
newDeepLink = """          if (url && url.includes('omnis://quote/')) {
...
if oldDeepLink in html:
    html = html.replace(oldDeepLink, newDeepLink)
    print("Deep link injected.")

# 2. Inject Team Tab Button
emailRecipientBtn = """                          <i class="fas fa-envelope"></i> Email Recipients
                        </button>"""
teamTabBtn = """                          <i class="fas fa-envelope"></i> Email Recipients
                        </button>
...
if emailRecipientBtn in html:
    html = html.replace(emailRecipientBtn, teamTabBtn)
    print("Team Tab Button injected.")

# 3. Inject Team Tab Content
tabContentAnchor = """<!-- TAB 1: CONNECTIVITY -->"""
teamTabContent = """
                      <!-- TAB: TEAM -->
...
"""
if tabContentAnchor in html:
    html = html.replace(tabContentAnchor, teamTabContent + '\n                      ' + tabContentAnchor)
    print("Team Tab Content injected.")

# 4. Inject Logic JS (Automation Call)
logicAnchor = """// Load data"""
logicJS = """
...
// Load data"""
if logicAnchor in html:
    html = html.replace(logicAnchor, logicJS)
    print("Logic injected.")

# 5. Inject Logic Functions and Modal at the bottom
logicFuncs = """
<script>
...
# USE RFIND TO ONLY REPLACE THE LAST OCCURRENCE
if 'id="sales-person-modal"' not in html:
    last_body_idx = html.rfind('</body>')
    if last_body_idx != -1:
        html = html[:last_body_idx] + logicFuncs + '\n' + html[last_body_idx:]
        print("Logic funcs and modal injected at the very end.")
    else:
