...
// Find the block from .omnis-top-nav to .top-nav-spacer
const startIndex = html.indexOf('.omnis-top-nav {');
const endIndexStr = '.top-nav-spacer {\n      flex: 1;\n    }';
const endIndexStrAlt = '.top-nav-spacer {\r\n      flex: 1;\r\n    }';

let endIndex = html.indexOf(endIndexStr);
if (endIndex === -1) {
    endIndex = html.indexOf(endIndexStrAlt);
}
if (endIndex === -1) {
    // manual fallback
    endIndex = html.indexOf('.top-nav-spacer');
...
if (startIndex !== -1 && endIndex !== -1) {
    ...
    fs.writeFileSync('systems/fleetrack/index.html', before + cssContent + after);
    console.log('Successfully updated navigation CSS in index.html');
} else {
    console.log('Failed to find CSS block boundaries', {startIndex, endIndex});
}
