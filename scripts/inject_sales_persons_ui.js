...
// Insert the new tab content just before the end of TABS CONTAINER
// Let's find "<!-- TAB 1: CONNECTIVITY -->" and insert it before that.
html = html.replace('<!-- TAB 1: CONNECTIVITY -->', teamTabContent + '\n                      <!-- TAB 1: CONNECTIVITY -->');
    ...
    <div style="background:#fff; width:100%; max-width:400px; border-radius:16px; box-shadow:0 20px 25px -5px rgba(0,0,0,0.1), 0 10px 10px -5px rgba(0,0,0,0.04); overflow:hidden;">
      ...
      <div style="padding:24px;">
        ...
        <div style="margin-bottom:16px;">
          ...
          <input type="text" id="sp-name" placeholder="e.g. Antony Dube" style="width:100%; box-sizing:border-box; padding:10px 12px; border:1px solid #cbd5e1; border-radius:8px; font-size:14px; outline:none; transition:all 0.2s;">
        </div>
        <div style="margin-bottom:16px;">
          <label style="display:block; font-size:12px; font-weight:600; color:#475569; margin-bottom:6px;">Email Address</label>
          <input type="email" id="sp-email" placeholder="antony@example.com" style="width:100%; box-sizing:border-box; padding:10px 12px; border:1px solid #cbd5e1; border-radius:8px; font-size:14px; outline:none; transition:all 0.2s;">
        </div>
        <div style="margin-bottom:16px;">
          <label style="display:block; font-size:12px; font-weight:600; color:#475569; margin-bottom:6px;">WhatsApp Number</label>
          <input type="text" id="sp-phone" placeholder="e.g. 263772000000" style="width:100%; box-sizing:border-box; padding:10px 12px; border:1px solid #cbd5e1; border-radius:8px; font-size:14px; outline:none; transition:all 0.2s;">
...
// Insert modal at the bottom near other modals
html = html.replace('<!-- --- QUICK ACTION MODALS --- -->', modalHTML + '\n<!-- --- QUICK ACTION MODALS --- -->');
...
html = html.replace('/* ---------- WHATSAPP AUTOMATION ---------- */', logicJS + '\n    /* ---------- WHATSAPP AUTOMATION ---------- */');

fs.writeFileSync(path, html);
console.log('injected Team settings successfully!');
