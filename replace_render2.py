...
replacement = r'''    function renderGroupSalesTable(rows) {
      ...
      rows.forEach(item => {
        ...
        row.className = "ai-sales-grid ai-order-row"; // Added ai-order-row so JS can find it
        row.style.cssText = "background:#fff; border-left:4px solid #8b2219; padding:10px 20px; border-bottom:1px solid #f1f5f9; cursor:pointer; transition:background 0.15s; min-height: 40px;";
      ...
      rowEls.forEach((rowEl, i) => {
        ...
        if (editBtn) editBtn.addEventListener('click', () => { if (typeof editGroupSale === 'function') editGroupSale(recordName); });
        if (deleteBtn) deleteBtn.addEventListener('click', () => { if (typeof deleteGroupSale === 'function') deleteGroupSale(recordName); });
      });
    }'''

import re
pattern = re.compile(r'    function renderGroupSalesTable\(rows\) \{.*?\}\n      \}\);\n    \}', re.DOTALL)
