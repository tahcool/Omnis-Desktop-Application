  ...
  window.marketingLoadHub = async function() {
      ...
      grid.innerHTML = '<tr><td colspan="5" style="padding:40px; text-align:center;"><i class="fas fa-spinner fa-spin"></i> Loading campaigns...</td></tr>';
      
      try {
          const res = await window.electron.ipcRenderer.invoke('supabase:query', {
              table: 'newsletters', method: 'select', params: { order: { column: 'created_at', ascending: false } }
          });
          if (!res.ok) {
              console.error('[Marketing] Load error:', res.error);
              grid.innerHTML = '<tr><td colspan="5" style="padding:40px; text-align:center; color:#ef4444;"><i class="fas fa-exclamation-triangle"></i> Error loading campaigns: ' + (res.error || 'Unknown') + '</td></tr>';
              return;
          }
          if (!res.data || res.data.length === 0) {
              grid.innerHTML = '<tr><td colspan="5" style="padding:40px; text-align:center; color:#94a3b8;">No campaigns yet. Click <strong>+ New Campaign</strong> to create one.</td></tr>';
              return;
          }
          grid.innerHTML = '';
          res.data.forEach(c => {
  ...
  if (!window.originalMarketingOpenBuilder) {
  ...
  // Note: marketingLoadHub is called via switchToView('view-marketing') — no need for a separate listener here.
