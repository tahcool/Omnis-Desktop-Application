...
class EmailManager {
  ...
  async _invokeEmailFunction(action, params = {}) {
    ...
    if (error) {
      // FunctionsHttpError wraps a Response object in error.context
      // We need to parse the body to get the real error message
      let message = error.message || 'Email operation failed';
      try {
  ...
  setupIPC() {
    ...
    // email:flushQueue removed — queue processing is entirely server-side
    // (pg_cron → process-email-queue Edge Function). No client control needed.
  }
}
