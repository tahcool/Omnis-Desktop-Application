...
const SERVICE_KEY  = process.env.SUPABASE_SERVICE_KEY || (p1 + p2);
...
class EmailManager {
  ...
  async _deliverNow({ queueId, to, cc, subject, html, plainText, attachmentUrls }) {
    try {
      ...
      const { data: row } = await this.supabase.from('omnis_email_queue').select('retry_count').eq('id', queueId).single();
      const currentRetries = (row && row.retry_count) || 0;
      const newRetries = currentRetries + 1;
      
      if (newRetries >= 5) {
        await this.supabase
          .from('omnis_email_queue')
          .update({ status: 'failed', error_message: e.message, retry_count: newRetries })
          .eq('id', queueId);
      } else {
        // Backoff: 5m, 15m, 45m, 135m
        const backoffMins = Math.pow(3, newRetries - 1) * 5; 
        const nextTime = new Date(Date.now() + backoffMins * 60000).toISOString();
