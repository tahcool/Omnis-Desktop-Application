...
class WhatsappManager {
    ...
    connect() {
        ...
        fs.appendFileSync(this.logPath, `[${new Date().toISOString()}] Client configured. Headless: new. ChromePath: ${chromePath}\n`);

        const logEvent = (name, details = '') => {
            const timestamp = new Date().toISOString();
            fs.appendFileSync(this.logPath, `[${timestamp}] EVENT: ${name} ${details}\n`);
        };
        ...
        this.client.initialize().then(() => {
            ...
            fs.appendFileSync(this.logPath, `[${new Date().toISOString()}] initialize() resolved\n`);
        }).catch(err => {
            ...
            fs.appendFileSync(this.logPath, `[${new Date().toISOString()}] ERROR: ${errorMsg}\nSTACK: ${stack}\n`);
