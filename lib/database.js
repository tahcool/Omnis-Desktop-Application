...
class LocalDatabase {
   ...
   */
  getAll(table) {
    const validTables = ['hot_leads', 'machine_stock', 'quotations', 'orders', 'customers', 'group_sales', 'enquiries', 'products', 'brands', 'item_groups', 'sync_queue'];
  ...
  /**
   * Search records in a table using JSON fields
   */
  search(table, query, fields) {
    const validTables = ['hot_leads', 'machine_stock', 'quotations', 'orders', 'customers', 'group_sales', 'enquiries', 'products', 'brands', 'item_groups'];
    if (!validTables.includes(table)) {
      throw new Error(`Invalid table: ${table}`);
    }
    ...
    try {
        ...
        const conditions = searchFields.map(f => `json_extract(data, '$.${f}') LIKE ?`).join(' OR ');
        const params = searchFields.map(f => `%${query}%`);

        const rows = this.db.prepare(`SELECT data FROM ${table} WHERE ${conditions} LIMIT 15`).all(...params);
        return rows.map(r => JSON.parse(r.data));
    } catch (e) {
        console.warn(`[LocalDB] JSON search failed, falling back to string match: ${e.message}`);
        // Fallback: Simple string search on the whole JSON blob
        const rows = this.db.prepare(`SELECT data FROM ${table} WHERE data LIKE ? LIMIT 15`).all(`%${query}%`);
        return rows.map(r => JSON.parse(r.data));
    }
  }
    ...
    for (let i = 0; i < total; i += CHUNK_SIZE) {
      ...
      const transaction = this.db.transaction((items) => {
        for (const record of items) {
          const name = record.name || record.id || record.frappe_id || `auto_${Date.now()}_${Math.random()}`;
   ...
   */
  getCount(table) {
    const validTables = ['hot_leads', 'machine_stock', 'quotations', 'orders', 'customers', 'group_sales', 'enquiries', 'sync_queue', 'products', 'brands', 'item_groups'];
    if (!validTables.includes(table)) {
      throw new Error(`Invalid table: ${table}`);
    }
  ...
  getStats() {
    return {
      ...
      brands: this.getCount('brands'),
      item_groups: this.getCount('item_groups'),
