...
async function initAllLifecycles() {
    console.log("Fetching all active quotes from Frappe Quotations with pagination...");
    ...
    while (true) {
        ...
        console.log("Fetched " + quotes.length + " quotes. Total so far: " + allQuotes.length);
        
        if (quotes.length < limit) break; // Reached the end
        
        start += limit;
    }

    console.log("Found " + allQuotes.length + " total active quotes. Initializing missing lifecycle tracking...");
    ...
    for (let i = 0; i < allQuotes.length; i += batchSize) {
        ...
        if (insertErr) {
            ...
            console.log("Processed " + initializedCount + "/" + allQuotes.length + " quotes...");
        }
    }
