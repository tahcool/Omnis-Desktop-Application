    ...
    async function loadFtDefects() {
      ...
      try {
        ...
        const { data, error } = await window.supabase.from('ft_defect').select('*').order('start_date', { ascending: false });
    ...
    async function loadDefectCategories() {
      ...
      try {
        const res = await callFrappe('frappe.client.get_list', { doctype: 'FT Defect Category', fields: '["name"]', limit_page_length: 500 }, 'GET');
    ...
    function renderDefectsTable(rows) {
      ...
      if (rows) {
        ...
        const open = rows.filter(r => (r.status || 'Open').toLowerCase() !== 'closed').length;
        const high = rows.filter(r => (r.priority || 'Low').toLowerCase() === 'high' && (r.status || 'Open').toLowerCase() !== 'closed').length;
    ...
    async function submitDefect() {
      ...
      try {
        ...
        const payload = {
          ...
          on_hold, ted_status, category, quotation_sent_date, parts_eta, ted, red, solution,
        ...
        if (!opts.includes(category)) {
           callFrappe('frappe.client.insert', { doc: { doctype: 'FT Defect Category', category_name: category } }, 'POST').catch(e => console.warn(e));
        }
