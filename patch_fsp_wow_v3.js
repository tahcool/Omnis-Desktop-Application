...
const start = lines.findIndex(l => l.includes('async function renderDashboardFsp()'));
const end = lines.findIndex((l, i) => i > start && l.includes('// 🗺️ Leaflet Map Logic'));

if (start !== -1 && end !== -1) {
  const newFuncLines = `    async function renderDashboardFsp() {
  ...
  lines.splice(start, end - start - 1, ...newFuncLines);
  fs.writeFileSync('systems/fleetrack/index.html', lines.join('\n'));
  console.log('Successfully applied WOW patch exactly to lines', start, 'to', end - 2);
} else {
