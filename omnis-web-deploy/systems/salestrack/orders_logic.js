...
function initOrdersLogic() {
    ...
    start.setMonth(today.getMonth() - 6); // Show last 6 months by default to capture all active orders
...
async function loadOrdersList(force = false) {
    ...
    container.innerHTML = `
        <div style="padding:60px; text-align:center; color:#94a3b8; font-weight:600; font-size:14px; background:white; border-radius:12px; border:1px dotted #cbd5e1;">
          <i class="fas fa-spinner fa-spin" style="margin-right:10px; color:#800000;"></i> Loading Order Tracking...
        </div>`;
    if (info) info.textContent = "Connecting to Supabase...";
    ...
    if (info) info.innerHTML = `<i class="fas fa-sync fa-spin"></i> Fetching <strong>${company || "All Companies"}</strong> from Supabase...`;

    try {
        // ─── PRIMARY SOURCE: omnis_tracking_orders (Supabase) ─────────────────
        const trackRes = await window.electron.invoke('supabase:query', {
            table: 'omnis_tracking_orders', method: 'select'
        });

        if (!trackRes.ok) throw new Error(trackRes.error || 'Failed to load orders from Supabase');

        const todayMs = new Date(); todayMs.setHours(0, 0, 0, 0);

        let ordersList = (trackRes.data || []).map(t => {
            let days_left = '-';
            if (t.target_handover) {
                const target = new Date(t.target_handover);
                target.setHours(0, 0, 0, 0);
                days_left = Math.ceil((target - todayMs) / 86400000);
            }
            return {
                supabase_id:       t.id,                                                          // raw UUID for saves
                report_id:         t.linked_sale_name || ('TRACK-' + t.id),                      // stable key for UI/localStorage
                machine_id:        t.id,
                customer:          t.customer,
        ...
        // ─── DATE FILTER (client-side on order_date) ───────────────────────────
        if (fromDate) {
            ordersList = ordersList.filter(o => !o.order_date || o.order_date >= fromDate);
        }
        if (toDate) {
            ordersList = ordersList.filter(o => !o.order_date || o.order_date <= toDate);
        }

        // ─── AUGMENT: Open Defect Counts (Supabase ft_defect) ─────────────────
        try {
            if (window.electron) {
                const defRes = await window.electron.invoke('supabase:query', {
                    table: 'ft_defect', method: 'select',
                    params: { columns: 'order_id, machine, status', filters: { status: 'Open' }, limit: 5000 }
                });
                if (defRes.ok && defRes.data) {
                    ...
                    ordersList.forEach(o => {
                        // match by supabase UUID (the order_id stored in ft_defect)
                        const defects = defectMap.get(o.supabase_id) || defectMap.get(o.report_id) || [];
                        o.open_defects_count = defects.length;
                    });
                }
            }
        } catch (e) { console.error('[OrdersLogic] Failed to augment defects', e); }
        ...
        const normalizeCompany = (c) => {
            ...
            return c; // trust whatever is stored in Supabase
        };
        ordersList.forEach(o => { o.company = normalizeCompany(o.company); });

        if (window.appendMissingCompanyFilters) {
            window.appendMissingCompanyFilters([...new Set(ordersList.map(o => o.company).filter(Boolean))]);
        }

        // ─── COMPANY FILTER ────────────────────────────────────────────────────
        if (company && company.toLowerCase() !== 'all') {
            const target = normalizeCompany(company);
            ordersList = ordersList.filter(o => o.company === target);
        }

        // ─── EXCLUDE DELETED ───────────────────────────────────────────────────
        ordersList = ordersList.filter(o =>
            o.status !== 'Deleted' && !localStorage.getItem('deleted_order_' + o.report_id)
        );

        olOrdersData            = ordersList;
        window.olOrdersData     = olOrdersData;
        if (window.dashManager) window.dashManager.ordersData = olOrdersData;

        if (ordersList.length > 0) {
            renderOrdersList();
            if (info) info.textContent = `${ordersList.length} orders`;
        } else {
        ...
        container.innerHTML = `
            <div style="padding:60px; text-align:center; color:#ef4444; background:#fff1f2; border:1px solid #fecaca; border-radius:12px;">
                <div style="font-size:18px; font-weight:800; margin-bottom:8px;">Order Sync Failed</div>
                <div style="font-size:13px; font-weight:500; opacity:0.8; margin-bottom:20px;">${err.message || "Unknown error"}</div>
                <button onclick="loadOrdersList(true)" style="padding:8px 24px; background:#ef4444; color:white; border:none; border-radius:8px; font-weight:700; cursor:pointer;">Retry</button>
            </div>`;
    }
}
...
window.setOrderCompany = async function(reportId, newCompany, selectEl) {
    ...
    try {
        // Always write to omnis_tracking_orders using the supabase UUID
        const order = olOrdersData.find(o => o.report_id === reportId);
        const dbId = order ? order.supabase_id : null;
        if (!dbId) throw new Error('Order not found in local data — refresh the list and try again.');
...
window.setOrderStatusInline = async function(reportId, machineId, newStatus, selectEl) {
    ...
    try {
        ...
        // Always write to omnis_tracking_orders using the supabase UUID
        const order = window.olOrdersData ? window.olOrdersData.find(o => o.report_id === reportId) : null;
        const dbId = order ? order.supabase_id : null;
        if (!dbId) throw new Error('Order not found in local data — refresh the list and try again.');
