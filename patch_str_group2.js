...
const startTarget = 'tbody.innerHTML = enrichedRows.map(obj => {';
const endTarget = "      }).join('');";
...
if (startIdx !== -1 && html.indexOf(endTarget, startIdx) !== -1) {
  const oldLogicFull = html.substring(startIdx, endIdx);
  ...
  console.log("Failed to find target rendering logic indices:", startIdx);
}
